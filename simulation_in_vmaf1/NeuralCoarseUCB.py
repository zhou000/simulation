...
# X_INFO = 7  # throughput*3, buffer_size, sum_chunk_size , sum_vmaf, sum_sm_penalty
# X_INFO = 5  # throughput*1, buffer_size, sum_chunk_size , sum_vmaf, sum_sm_penalty
X_D = X_INFO
X_LEN = 8
...
# confidence_scaling_factor = 0.001
# confidence_scaling_factor = 0.0001
# confidence_scaling_factor = 0.00001
# confidence_scaling_factor = 0
training_window = 100
# p = 0.0
p = 0.2
learning_rate = 0.01
...
def main():
    ...
    ctx = np.zeros((T, n_arms, n_features))
    rewards = np.zeros((T, n_arms))
    ...
    while True:  # serve video forever
        ...
        if video_chunk_num > 0:
            ...
            # reward_t = reward
            reward_t = coarse_reward
            iterarion_id = video_chunk_num - 1
            rewards[iterarion_id] = reward_t

            # # update approximator
            # if video_chunk_num % train_every == 0:
            if iterarion_id % train_every == 0:
                """Train neural approximator.
                        """
                iterations_so_far = range(np.max([0, iterarion_id - training_window]), iterarion_id + 1)
                actions_so_far = actions[np.max([0, iterarion_id - training_window]):iterarion_id + 1]
        ...
        '''
        the desicion for the next video chunk
        '''
        ...
        if video_chunk_num >= 0:
            ...
            for i in range(0, A_DIM):
                ...
                # cx_i = np.array(
                #     [start_buffer, throughput_1, sum_video_quality, sum_future_chunk_size, sum_sm_penalty])
                # cx_i = np.array(
                #     [start_buffer, throughput_1, throughput_2, throughput_3, sum_video_quality, sum_future_chunk_size, sum_sm_penalty])
                cx_i = np.array(
            ...
            confidence_multiplier = confidence_scaling_factor * np.sqrt(approximator_dim * np.log(
                1 + video_chunk_num * bound_features ** 2 / (reg_factor * approximator_dim)) + 2 * np.log(1 / delta))
            ...
            model.eval()
            mu_hat[video_chunk_num] = model.forward(torch.FloatTensor(context).to(device)).detach().squeeze()
