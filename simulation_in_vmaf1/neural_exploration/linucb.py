...
class LinUCB(UCB):
    ...
    def train(self):
        ...
        # self.b[self.action] += context[self.action]*self.bandit.rewards[self.iteration, self.action]
    ...
    def predict(self):
        ...
        self.mu_hat[self.iteration] = np.array(
            [
                ...
                # np.dot(context[a], self.theta[a]) for a in self.bandit.arms
            ]
        )
