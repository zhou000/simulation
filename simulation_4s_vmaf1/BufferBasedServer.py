...
def main():
    ...
    while True:  # serve video forever
        ...
        if buffer_size < reservoir:
            BB_bitrate = VIDEO_BIT_RATE[0]
        elif buffer_size > reservoir + cushion:
            BB_bitrate = VIDEO_BIT_RATE[-1]
        else:
            BB_bitrate = VIDEO_BIT_RATE[0] + (VIDEO_BIT_RATE[-1] - VIDEO_BIT_RATE[0]) * (
                    buffer_size - reservoir) / cushion

        # quality = bisect.bisect_right(VIDEO_BIT_RATE, BB_bitrate) - 1
        # quality = max(quality, 0)

        for i in range(len(VIDEO_BIT_RATE)-1, -1, -1):
            if BB_bitrate >= VIDEO_BIT_RATE[i]:
                BB_action = i
                break
            BB_action = i

        # BB_action = max(BB_action, 0)
