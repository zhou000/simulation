...
X_INFO = 3  # throughput*1, bit_rate, buffer_size
X_D = X_INFO
# X_LEN = 3
X_LEN = 8
...
def main():
    ...
    while True:  # serve video forever
        ...
        x_context[2, -1] = state[3, -1]  # throughput    kilo byte / ms
        # x_context[2, -1] = state[3, -1] / 10  # throughput    kilo byte / ms  /10
        ...
        for i in range(0, A_DIM):
            ...
            cx_i = np.array(
                [bitrate_i, start_buffer, throughput_1])
            x = cx_i
            x = np.array(x).reshape((X_D, 1))
            x_t = np.transpose(x)
            index = i
