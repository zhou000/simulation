...
def main():
    ...
    while True:  # serve video forever
        ...
        # BB + LinUCB
        # BB_buffer = float(post_data['buffer'])
        BB_buffer = buffer_size
        # last_arm = post_data['lastquality']
        last_arm = bit_rate
        # print("max_A, Buffer, Rebuffer_time: ", max_A, BB_buffer, rebuffer_time)
        # if BB_buffer < 4.0 or rebuffer_time > 0:
        if BB_buffer < 4.0 or rebuf > 0:
            idx = 0
        # elif BB_buffer < 8.0 and idx > 2:
        elif BB_buffer < BUFFER_BOUND1 and idx > 2:
            if last_arm <= 2:
                idx = last_arm
            else:
                idx = 2
        # if BB_buffer > 20.0 and idx <= 2:
        #     if last_arm > 2:
        #         idx = last_arm
        #     else:
        #         idx = 3
